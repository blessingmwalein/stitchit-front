<template>
    <Modal v-if="isFinishedProductFormModal" @close="closeModal()">
        <template #header>
        ...
        <template #body>
            <div class="no-scrollbar relative w-full overflow-y-auto bg-white dark:bg-gray-900">
                ...
                <div class="card flex flex-col items-center gap-6 w-full mb-4 mt-4">
                    ...
                    <FileUpload title="Choose Design" mode="basic" @select="onFileSelect" customUpload auto
                ...
                <client-only>
                    <Vueform @submit="handleSubmit" v-model="finishedProductForm" :show-errors="false"
                        ...
                        <SelectElement label="Unit" name="unit" :native="false" :rules="['required']"
                            :columns="{ container: 6 }" :items="unitsData" />
                            <SelectElement label="Shape" name="shape" :native="false" :rules="['required']"
                        ...
                        <ButtonElement name="close" button-class="bg-red-500" :columns="{ container: 4 }"
                            @click="closeModal()" button-label="Close" :full="true" size="lg" />
                        <ButtonElement button-class="bg-brand-500" name="register" :columns="{ container: 4 }"
...
const finishedProductForm = computed({
  ...
  set: (data) => finishedProductStore.createFinishedProductForm = data
});
...
const handleSubmit = async () => {
  try {
    if (finishedProductStore.selectedProduct?.id) {
      ...
      await finishedProductStore.createOrder();
    }
