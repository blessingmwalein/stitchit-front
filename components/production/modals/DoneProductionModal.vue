        ...
        <template #body>
            <div class="no-scrollbar relative w-full  overflow-y-auto bg-white  dark:bg-gray-900">
                <client-only>
                            ...
                            <table class="min-w-full text-sm text-left text-gray-600 dark:text-gray-300">
                                ...
                                <tbody>
                                    <tr v-for="item in group.items" :key="item.name + item.description"
                                        ...
                                        <td class="py-1 pr-3">${{ item.unitCost.toFixed(2) }}</td>
                                        <td class="py-1 pr-3">${{ item.cost.toFixed(2) }}</td>
                                    </tr>
                                    <tr class="font-semibold">
                                        <td colspan="5" class="text-right py-2 pr-3">Total</td>
                                        <td class="py-2 pr-3">${{ group.totalCost.toFixed(2) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <Vueform @submit="handleSubmit" v-model="updateProductionForm" :show-errors="false"
