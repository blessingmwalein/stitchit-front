<template>
  <Modal v-if="isClientFromModal" @close="closeModal()">
    <template #header>
      ...
      <div class="px-2 pr-14">
        <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
          {{ clientProfile ? `Edit Client Profile - ${clientProfile.name}` : 'Add New Client' }}
        </h4>
...
const emit = defineEmits(['update:isClientFromModal']);
const clientStore = useClientsStore();
const errors: Record<string, string> = clientStore.errors;
...
const clientForm = computed({
  get: () => clientStore.createClientForm,
  set: (data: CreateClientRequest) => clientStore.createClientForm = data
})
...
const handleSubmit = async () => {
  ...
  console.log(clientStore.createClientForm);

  try {
    if (props.clientProfile != null) {
      await clientStore.updateClientProfile();
    } else {
    ...
    if (clientStore.errorMessage) {
      snackbar.add({
        type: "error",
        text: clientStore.errorMessage,
      });
    }
  }
