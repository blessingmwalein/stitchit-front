<template>
  <Modal v-if="isViewRugModal" @close="closeModal()">
    <template #header>
      <div class="flex items-center justify-between w-full">
        ...
        <div class="flex gap-3 mr-3">
          <CustomButton @click="editRug()" type="submit" label="Edit Rug" variant="brand" rounded="full"
              ...
              <div>
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                  <div>
                    ...
                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{rug?.name}}</p>
                  </div>
    
                  <div>
                    <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Type</p>
                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{rug?.type}}</p>
                  </div>
    
    
    
                  <div>
                    <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Cost</p>
                  ...
                  <div>
                    <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Production Cost</p>
                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                      ${{ rug?.approx_production_cost }}
                    </p>
...
import RugFormModal from './RugFormModal.vue';
import type { Rug } from '~/utils/models/rug';


const emit = defineEmits(['update:isViewRugModal', 'edit']);
...
//cl
const closeModal = () => {
  emit('update:isViewRugModal', false);
};

const firstName = computed(() => {
  return props.rug?.name.split(' ')[0]
})

//get last name from name user pfile

const lastName = computed(() => {
  return props.rug?.name.split(' ')[1]
})
