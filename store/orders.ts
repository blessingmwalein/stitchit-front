...
import type { OrderData, OrdersResponse, CreateOrderRequest, CreateOrderResponse, Order } from "~/utils/models/order";
...
export const useOrderStore = defineStore("order", {
    ...
    actions: {
        ...
        async updateOrder() {
            ...
            if (!this.token) return;
            try {
                const response = await fetch(getApiUrl(`client/orders/${this.selectedOrder?.id}`), {
                    method: "PUT",
                    body: {
